...
def guide_me():
    ...
    results_container.write(selected_json_content['reasoning'])
    results_container.markdown(f"*CGPA: {student_cgpa}*")
    results_container.markdown(f"*Current Semester: {credits_json_content['current_semester']}*")
    ...
    for course in selected_json_content['selected_courses']:
